    ...
    def add_miles_arguments(parser):
        ...
        def add_train_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Flow-matching shift for the training-side sigma grid, regenerated by "
                    "the trainer when no rollout engine supplies scheduler meta (SFT). "
        ...
        def add_rollout_arguments(parser):
            ...
            parser.add_argument(
                ...
                type=int,
                default=1,
                help="Frame count of the trained media: decoded rollout frames, SFT encode frames per clip. Wan2.2 MVP uses 1.",
            )
            parser.add_argument(
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                help=(
                    "How rollout train pairs are dealt to DP ranks. contiguous keeps each rank's "
                    "samples adjacent, so a micro-batch can reproduce a rollout microgroup exactly. "
                    "baseline_stride deals samples round-robin, reproducing the miles-core dispatch "
                    "that strides for sequence-length balance -- diffusion samples are equal-sized, "
                    "so it is only useful for the legacy parity check."
                ),
            )
            parser.add_argument(
        ...
        def add_algo_arguments(parser):
            ...
            parser.add_argument(
                "--loss-type",
                type=str,
                choices=["policy_loss", "sft_loss", "custom_loss", "nft"],
                default="policy_loss",
                help=(
