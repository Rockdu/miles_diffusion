...
class Wan2_2TrainPipelineConfig(TrainPipelineConfig):
    ...
    # normed value to bf16 mid-chain and inject ~3e-3 rel per site against the
    # trainer; eager costs rollout speed, which parity dumps accept.
    rollout_patch_group = "wan"
    # Boundary inputs, verified against paired sglang-d dumps: the rollout casts
    # the latent to the forward dtype but passes the T5 text embeds through in
    # fp32 (its first context linear consumes an fp32 input with bf16 weights)
    # and keeps the raw timestep fp32. The timestep axis is load-bearing: bf16
    # rounds 857.6923 to 856.0, which moves temb 5.4e-3 off the rollout and is
    # the first divergence in an otherwise bitwise chain.
