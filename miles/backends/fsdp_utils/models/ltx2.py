...
Reference implementation for onboarding a self-built model: wraps the
official ``ltx_core`` transformer so the trainer can drive it exactly like a
diffusers model (see ``models/__init__.py`` for the protocol contract).
"""
...
class LTX2TransformerModeling:
    """Diffusers-protocol facade over the ltx_core transformer.

    Native (self-built) modeling exposes the same interface guarantees a
    diffusers ModelMixin gives the trainer — ``from_pretrained``,
    ``_no_split_modules`` and ``enable_gradient_checkpointing()`` — so
    ModelBackend treats both modeling mechanisms identically.
    """
    ...
    def from_pretrained(
        ...
        *,
        ...
        checkpoint = resolve_transformer_checkpoint(str(ref), explicit_path=weights_path)
        model = load_ltx_transformer_for_train(checkpoint, device=device, dtype=torch_dtype)
        model._no_split_modules = list(cls._no_split_modules)
        if not hasattr(model, "enable_gradient_checkpointing"):
            if hasattr(model, "set_gradient_checkpointing"):
                model.enable_gradient_checkpointing = partial(model.set_gradient_checkpointing, True)
            else:
                raise AttributeError(
                    "LTX transformer exposes neither enable_gradient_checkpointing "
                    "nor set_gradient_checkpointing"
                )
        return model
