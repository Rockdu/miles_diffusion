...
def build_fsdp_meshes(
    ...
    shard_view = world_mesh._unflatten(0, (dp_replicate, world_size // dp_replicate), ("dp_replicate", "fsdp_shard"))
    # A degree-1 replicate axis would all-reduce over a single rank every bucket.
    fsdp_mesh = shard_view if dp_replicate > 1 else shard_view["fsdp_shard"]
