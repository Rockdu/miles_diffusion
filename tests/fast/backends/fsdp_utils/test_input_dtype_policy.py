...
def test_default_policy_is_passthrough():
    """The base default pins nothing, so every float input keeps the dtype it arrived with."""
...
def test_cast_policy_casts_floats_only():
    """A family that pins latents/cond gets them cast to the forward dtype; masks stay int."""
    latents, timesteps, sigmas, pos_cond = _inputs()
