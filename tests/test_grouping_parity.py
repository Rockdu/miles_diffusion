...
def test_l2_converter_pairs_match_direct_indexing():
    ...
    for si, s in enumerate(samples):
        ...
        for t_pos, idx in enumerate(sde):
            p = pairs[k]
            k += 1
            # per-sample fields
            ...
            assert torch.equal(d["rollout_step_prev_sample_mean"], s.rollout_debug_tensors.rollout_prev_sample_means[idx])
