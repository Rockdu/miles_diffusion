...
def plot_all(out_dir: Path, train_rows: list[dict], perf_rows: list[dict], reward_rows: list[dict]) -> list[Path]:
    ...
    if train_rows and reward_rows:
        ...
        fig.suptitle(out_dir.parent.name, fontsize=10)
        fig.tight_layout()
...
def main() -> None:
    ...
    parser.add_argument("--run-marker", type=str, default=RUN_MARKER)
    parser.add_argument("--log-file", type=Path, default=None, help="parse this driver log directly instead of hunting Ray session logs")
    args = parser.parse_args()
    if args.run_dir is None:
        args.run_dir = Path(f"/root/miles_diffusion/logs/{args.run_marker}")

    marker = args.run_marker
    if args.log_file is not None:
        train_log = reward_log = args.log_file
    else:
        found = find_ray_logs(args.ray_session, marker)
        if found is None:
            raise SystemExit(f"No Ray logs found containing checkpoint path for {marker}")
