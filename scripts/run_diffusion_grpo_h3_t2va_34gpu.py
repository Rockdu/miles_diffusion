"""MiniMax H3 t2va PickScore Flow-GRPO: 4 nodes x 8 GPU train+rollout + 2 reward GPUs.
...
Multi-node: one command on the head node builds the whole cluster and trains. The head
starts ray locally, joins every hostfile node over ssh (port 2222; run ops/bootstrap_ssh.sh
once per container lifetime), and submits. A hostfile line is 'ip [num_gpus]' -- list the
head too; the 2-GPU reward node uses the second column. The reward workers are
default-scheduled, so they land on the only GPUs the engines do not occupy.
...
Usage (on the head node):
    MASTER_ADDR=<head_ip> python3 scripts/run_diffusion_grpo_h3_t2va_34gpu.py
...
def execute(args: ScriptArgs, prompt_dir: str) -> None:
    ...
    run_name = f"diffusion_grpo_h3_t2va_34gpu_{U.create_run_id()}"
