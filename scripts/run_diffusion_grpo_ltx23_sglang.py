...
57-frame 512x768 video at 24 fps, 24 denoising steps, CPS-SDE with 3 trainable steps
drawn per epoch from candidate steps 0-9. Everything runs bf16 end to end (master,
reduce, forward, and the sgl-d engine) on the sdpa_math attention backend.

--hf-checkpoint points at gpt2 on purpose: LTX-2.3 needs no HF tokenizer here, and the
flag still requires a resolvable repo id.
...
def execute(args: ScriptArgs):
    ...
    ckpt_args = (
        "--hf-checkpoint gpt2 " f"--save {args.output_dir}/{run_name}/ckpt " "--save-interval 50 "
    )
    ...
    lora_args = (
        "--use-lora " "--lora-rank 64 " "--lora-alpha 128 " "--diffusion-init-lora-weight gaussian "
    )
    ...
    wandb_args = U.get_default_wandb_args(WANDB_PROJECT, run_name, log_images=4)
    ...
    # Video rollouts take minutes per request, so the health checker needs a far
    # longer interval and failure budget than the image recipes.
    misc_args = (
        "--actor-num-nodes 1 "
